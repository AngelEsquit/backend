...
func main() {
	...
	r.Post("/register", handlers.PostRegisterHandler(db)) // Ruta para registrar nuevos usuarios
	r.Post("/login", handlers.PostLoginHandler(db))       // Ruta para iniciar sesión

	// Ruta para obtener datos de usuario (protegida más tarde con JWT)
	// Por ahora, cualquiera puede acceder si conoce el ID
	r.Get("/users/{userID}", handlers.GetUserHandler(db)) // Ruta para obtener info de usuario
